...
CREATE TABLE IF NOT EXISTS assets (
    ...
    asset_fingerprint_hash VARCHAR(64) UNIQUE,
    created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS listings (
    ...
    condition_grade VARCHAR(32),
    seller_id VARCHAR(128),
    seller_feedback_score INT,
    seller_feedback_percentage NUMERIC(6,3),
    seller_location VARCHAR(128),
...
CREATE TABLE IF NOT EXISTS listing_snapshots (
    ...
    listing_id VARCHAR(128) REFERENCES listings(listing_id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS comp_sets (
    ...
    listing_id VARCHAR(128) REFERENCES listings(listing_id) ON DELETE CASCADE,
    raw_comps_count INT,
    rejected_comps_count INT,
    used_comps_count INT,
    median_comp_price NUMERIC(10,2),
    weighted_comp_price NUMERIC(10,2),
    condition_penalty_pct NUMERIC(5,4),
    fast_sale_estimate NUMERIC(10,2),
    conservative_resale_estimate NUMERIC(10,2),
    expected_resale_estimate NUMERIC(10,2),
    optimistic_resale_estimate NUMERIC(10,2),
    confidence_score NUMERIC(5,4),
    comp_evidence_payload JSONB,
...
CREATE TABLE IF NOT EXISTS decisions (
    ...
    confidence_score NUMERIC(5,4),
    safe_ceiling NUMERIC(10,2),
    normal_ceiling NUMERIC(10,2),
    aggressive_ceiling NUMERIC(10,2),
    expected_net_profit NUMERIC(10,2),
    evidence_ledger JSONB NOT NULL,
    evaluated_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE TABLE IF NOT EXISTS auction_events (
    auction_id VARCHAR(128) PRIMARY KEY REFERENCES listings(listing_id) ON DELETE CASCADE,
    end_time TIMESTAMPTZ NOT NULL,
    state VARCHAR(32) NOT NULL,
    alert_offsets INT[] DEFAULT ARRAY[86400,3600,900,300],
